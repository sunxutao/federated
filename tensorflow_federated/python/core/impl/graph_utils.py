...
# # TODO(b/122081673): Currently get_tf_typespec_and_binding and
# get_tf2_result_dict_and_binding don't have direct unit tests,
# but have full coverage via tests in api/computation_tests.py.
# However, it would be better to have proper unit tests.


# XXX Q - The TF2 helpers in this file aren't really graph related, even
# though they are very analogous to existing functions,
# perhaps this file should be called tensorflow_utils?
...
# XXX This needs to be updated along the lines of the changes
# to capture_result_from_graph in cl/245791603.
def get_tf2_result_dict_and_binding(result, name_prefix=None):
  """Returns the result_dict, type, and bindings for this result.

  This is the TF2 equivalent of `capture_result_from_graph`.
  ...
  Returns:
    A tuple (result_dict, result_type, binding), where
      ...
      binding: For mapping result_dict to the result_type.
